...
void verify_test(SSL *ssl);
void verify_chain(STACK_OF(X509) *certs);
...
void verify_chain(STACK_OF(X509) *certs)
{
  ...
  BIO              *certbio = NULL;	
	...
	certbio = BIO_new(BIO_s_file());
	ret = BIO_read_filename(certbio, "cert.pem");//CERTIFICATE FILE NAME.
  if (! (cert = PEM_read_bio_X509(certbio, NULL, 0, NULL))) {
	...
	X509_STORE_CTX_init(vrfy_ctx, NULL, cert, NULL);
	X509_STORE_CTX_trusted_stack(vrfy_ctx, certs);
...
void verify_test(SSL *ssl)
{
	...
	printf("verify_test begin.\n");
	verify_chain(certs);
	printf("verify_test end.\n");
}
